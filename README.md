...
- [ ] **Phase 0** — Project scaffolding (`create-next-app` with TypeScript, Tailwind, App Router, ESLint, Prettier, `.env.example`, clean folder architecture)
- [ ] **Phase 1** — README foundation (Project title, one-line description, tech stack badges, "🚧 In Progress" status badge, empty progress checklist listing all 55 phases)
- [ ] **Phase 2** — TypeScript strict mode + path aliases (`strict: true`, `@/` import alias, base types in `types/`)
- [ ] **Phase 3** — Environment configuration (`.env.example`, runtime Zod validation in `lib/env.ts`, `.gitignore` verification)
